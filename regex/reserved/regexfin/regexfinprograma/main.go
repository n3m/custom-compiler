...
//NewRegexFinPrograma ...
func NewRegexFinPrograma(EL, LL, GL *log.Logger) (*RegexFinPrograma, error) {
	var moduleName string = "[RegexFinPrograma][NewRegexFinPrograma()]"

	if EL == nil || LL == nil || GL == nil {
		return nil, fmt.Errorf("[ERROR]%+v Loggers came empty", moduleName)
	}
	return &RegexFinPrograma{
		Keyword: "Fin de Programa",
		V1:      regexp.MustCompile("^(?i)Fin de Programa"),
		V2:      regexp.MustCompile("^(?i)Fin de Programa"),
		V3:      regexp.MustCompile("^(?i)Fin de Progra"),
		V4:      regexp.MustCompile("^(?i)Fin de Prog"),
		GL:      GL,
		EL:      EL,
		LL:      LL,
	}, nil
...
//StartsWithFinPrograma ...
func (r *RegexFinPrograma) StartsWithFinPrograma(str string, lineIndex int64) bool {
...
//StartsWithFinProgramaNoCheck ...
func (r *RegexFinPrograma) StartsWithFinProgramaNoCheck(str string) bool {
	if r.V1.MatchString(str) {
		return true
	}
...
func (r *RegexFinPrograma) LogError(lineIndex int64, columnIndex interface{}, err string, description string, currentLine string) {
